...
python3 encode.py --renders /path/to/pro2 --out /path/to/pro2/assets
...
for mode, name, bg in video_specs:
    ...
    with tempfile.TemporaryDirectory(prefix='pro2-encode-') as tmp:
        for frame in frames:
            rgba = grade(frame)
            alpha = rgba[:, :, 3:] / 255.0
        ...
        subprocess.run(['ffmpeg', '-v', 'error', '-y', '-framerate', '60', '-i', f'{tmp}/%04d.png',
                        ...
                        '-movflags', '+faststart',
                        str(a.out / f'airpods_pro_2_{name}.mp4')], check=True)
...
for mode, suffix in [('assembly', ''), ('buds', '_buds'), ('case', '_case'), ('left', '_left'), ('right', '_right')]:
    ...
    if mode in ['left', 'right']:
        ...
        image = canvas
    image.save(a.out / f'airpods_pro_2{suffix}.png')
    print('STILL', mode, flush=True)

# Bluetooth metadata consumes a standalone icon, not the composed popup layer.
case = Image.open(a.out / 'airpods_pro_2_case.png')
case.crop(case.getchannel('A').getbbox()).save(a.out / 'airpods_pro_2_case_icon.png')
