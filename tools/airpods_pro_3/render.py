"""Render the Pro 3 rig to transparent PNGs; encode.py makes Android assets."""
import argparse
import math
import json
import sys
...
# Pose each complete unit independently; screenshot shows less exposed inner tip.
left.rotation_euler.z = math.radians(-20)
right.rotation_euler.z = math.radians(20)
...
if a.mode == 'connected':
    ...
    phase = math.radians(layout['phase_degrees']) if a.reference_phase else 0

def hide_group(key):
    for o in bpy.data.collections['Pro3_' + key].objects: o.hide_render = True
...
for side in ['Left', 'Right']:
    for obj in bpy.data.collections['Pro3_' + side].objects:
...
# Imported AR custom normals flatten broad shell regions under studio lights.
# Recalculate only the plastic-shell normals; vertex positions stay untouched.
for obj in scene.objects:
    if obj.type == 'MESH' and any(m.name.startswith('WwrjthXqFCMqrXz') for m in obj.data.materials):
...
shared = apply_shared_studio('airpods_pro_3', a.mode)
