"""Import Apple's Pro 3 USDZ and build an editable, independently articulated rig.

blender -b --python build.py -- --source apple_airpods_pro_3.usdz --out airpods_pro_3.blend
...
# Use the source hierarchy, never spatial thresholds: silicone tips extend into
# the case's X band. These four roots partition all 55 source meshes exactly.
source_roots = {'Left': 'BLvfImZvTRtJYtd', 'Right': 'bqtvQbjelRVbbzz',
                'Body': 'ZnPUFrNzvqVdFKc', 'Lid': 'kpaqGvvhsGowTJD'}
...
pin = center([bpy.data.objects['nnrNTFKuytzaDJI']])
...
for key, objects in groups.items():
    collection = bpy.data.collections.new('Pro3_' + key)
