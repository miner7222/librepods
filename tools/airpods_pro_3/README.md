...
The first connection frame is front-facing (0°); the reference phase is used only for comparison. A still screenshot
...
clipping; bounds at the reference angle must be within five source-image pixels of the
...
Connection/island minimum margins are 36/48 px. Popup product bounds differ
by at most 4.18 source pixels.


## Front-facing animation start

Production connection videos always start at 0° (front) and complete one turn
in 360 frames. The measured screenshot phase affects comparison only, not the
animation start. Use `--reference-phase --frames 1` for a like-angle inspection
render. The popup comparison uses the corresponding rounded video frame (modulo
360), while preserving calibrated camera, spacing, materials and grade.


## Pro 3 reference recheck

The case remains front-facing at production frame zero. Each complete unit has
a local yaw correction (left −20°, right +20°) to match the reference's straight
stems and reduced inner-tip exposure at the 180° comparison frame. Mesh vertices
are unchanged. Reduced ambient fill and lower reflection cards eliminate the
bright arch across the case top. Color calibration was recomputed for that studio.
