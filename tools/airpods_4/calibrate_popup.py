...
ref=np.array(Image.open(a.work/'refs'/f['reference_filename']).convert('RGB'))
...
(a.tools/'color-calibration.json').write_text(json.dumps({'percentiles':levels,'raw_case_rgb':raw.tolist(),'popup_case_rgb':target.tolist(),'reference':f.get('source_url', f['reference_filename']),'method':'Inset superellipse mask in case bounding box; low anchors preserve dark sensors; RGB curve does not replace physical gloss lighting.'},indent=2)+'\n')
