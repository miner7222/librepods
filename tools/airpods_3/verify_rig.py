...
blender -b airpods_3.blend --python verify_rig.py -- report.json
"""
import json
import math
import sys
...
body = list(bpy.data.collections['AirPods3_Body'].objects)
lid_parts = list(bpy.data.collections['AirPods3_Lid'].objects)
left_parts = list(bpy.data.collections['AirPods3_Left'].objects)
right_parts = list(bpy.data.collections['AirPods3_Right'].objects)
assert len(body + lid_parts + left_parts + right_parts) == 43
...
for angle in [0, 15, 30, 60, 90, 110, 60, 0]:
...
# Published case dimensions: 54.4 x 21.38 x 46.4 mm. Allow 0.4 mm for the AR
# representation and the evaluated outer-shell subdivision surface.
assert all(abs(x - y) < .0004 for x, y in zip(dims, [.0544, .02138, .0464])), dims
assert len([im for im in bpy.data.images if im.source == 'FILE']) > 0
assert all(im.packed_file for im in bpy.data.images if im.source == 'FILE'), 'unpacked texture'
report = {'meshes': 43, 'parts': {k: len(bpy.data.collections['AirPods3_' + k].objects)
