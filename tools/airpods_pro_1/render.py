"""Render the AirPods Pro 1 rig to transparent PNGs; encode.py makes Android assets."""
import argparse
import math
import json
import sys
...
p.add_argument('--reference-phase', action='store_true', help='Inspection only: use the reference screenshot rotation instead of front-facing start')
...
left.rotation_euler = (Matrix.Rotation(math.radians(-6), 3, 'Y') @ left.rotation_euler.to_matrix()).to_euler()
right.rotation_euler = (Matrix.Rotation(math.radians(6), 3, 'Y') @ right.rotation_euler.to_matrix()).to_euler()
...
if a.mode == 'connected':
    ...
    phase = math.radians(layout['fit_phase_degrees']) if a.reference_phase else 0

def hide_group(key):
    for o in bpy.data.collections['Pro1_' + key].objects: o.hide_render = True
...
for material in bpy.data.materials:
    if material.name in ['AWTzhuHRqcuHHac']:
...
# Soft silicone has no lacquered highlights; split it from the shared AR plastic.
for name in ['Left_FUtvjnzouomCSDN','Right_HJTmepUhNpWfTur']:
...
# Recompute shell normals while preserving seam vertices; imported broad-face
# custom normals otherwise produce a bright arch across the lid.
for name in ['Body_tzlGmjnaIfaDljJ','Body_cDHOoqUnmYIIoFS','Lid_fjGuSyuIIpNtmsC']:
    obj=bpy.data.objects[name]
...
area('Key_Softbox', (-.12, -.35, .42), 4.0, .5)
...
for name, x in [('Left_Reflection', -.20), ('Right_Reflection', .20)]:
    area(name, (x, -.42, -.12), .55, .12)
    bpy.data.lights[name].size_y = .35
...
# One metric presentation contract for stills and motion across generations.
sys.path.insert(0, str(Path(__file__).resolve().parents[1]))
from apple_artwork.studio import apply as apply_shared_studio
shared = apply_shared_studio('airpods_pro_1', a.mode)
if shared[0] is not None:
    width, height, ortho, elev, target = shared
if a.mode == 'connected' and not a.reference_phase: phase = 0
...
# Record completion only after every requested frame was written.
