...
blender -b airpods_pro_1.blend --python verify_rig.py -- report.json
"""
import json
import math
import sys
...
body = list(bpy.data.collections['Pro1_Body'].objects)
lid_parts = list(bpy.data.collections['Pro1_Lid'].objects)
left_parts = list(bpy.data.collections['Pro1_Left'].objects)
right_parts = list(bpy.data.collections['Pro1_Right'].objects)
assert len(body + lid_parts + left_parts + right_parts) == 84
...
report = {'meshes': 84, 'parts': {k: len(bpy.data.collections['Pro1_' + k].objects)
...
plates=[o for o in scene.objects if 'FlushHinge_' in o.name]
assert len(plates)==4
for o in plates:
    depths=[(o.matrix_world @ v.co).y for v in o.data.vertices]
    assert max(depths)-min(depths)<.000121, o.name
